...
def sortEvents(parent):
    ...
    events = readJson("code/running_events.json")
...
def resizeList(parent, value=None):
    childCount = len(readJson("code/running_events.json")) if value == None else value
...
class RunningEventList(StackLayout):
    ...
    def update(self, load=False, value=None):
        if load:
            ...
            writeJson("code/running_events.json", [])
            ...
            if flag:
               deleteAll(self)
               writeJson("code/running_events.json", value)
               for e in value:
                   self.add_widget(RunningEvent(e))
            else:
                writeJson("code/running_events.json", running)
